...
class Database:
    ...
    async def add_subscription(
        ...
        async with await self._connect() as db:
            await db.execute(
                """
                INSERT INTO subscriptions (user_id, tier, expires_at, payment_method, tx_id)
                VALUES (?, ?, ?, ?, ?)
                """,
    ...
    async def create_session(self, user_id: int, total_emails: int) -> int:
        async with await self._connect() as db:
            cursor = await db.execute(
                "INSERT INTO sessions (user_id, total_emails) VALUES (?, ?)",
    ...
    async def complete_session(self, session_id: int, result_file: str):
        async with await self._connect() as db:
            await db.execute(
                """
                UPDATE sessions
                SET completed_at = ?, result_file = ?
                WHERE id = ?
                """,
                (datetime.utcnow(), result_file, session_id),
            )
            await db.commit()
