...
class Database:
def init(self, db_path: str = config.DATABASE_PATH):
...
async def init(self):
    async with await self._connect() as db:
        ...
        if "locked_count" not in columns:
            await db.execute(
                "ALTER TABLE sessions ADD COLUMN locked_count INTEGER DEFAULT 0"
            )
...
async def add_subscription(
    ...
    async with await self._connect() as db:
        await db.execute(
            ...
            (user_id, tier, expires_at, payment_method, tx_id)
            VALUES (?, ?, ?, ?, ?)
            """,
            (
                user_id,
